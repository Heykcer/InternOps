...
async function routes(fastify) {
  ...
  // Department attendance report (admin only)
  fastify.get('/department-attendance', { preHandler: [auth, rbac('ADMIN')] }, async (req) => {
    ...
    `;
    const params = [];
    if (from) { query += ` AND a.date >= ${params.length+1}`; params.push(from); }
    if (to) { query += ` AND a.date <= ${params.length+1}`; params.push(to); }
    if (departmentId) { query += ` AND d.id = ${params.length+1}`; params.push(departmentId); }
    query += ` GROUP BY d.id, d.name ORDER BY d.name`;
    const { rows } = await pool.query(query, params);
    return rows;
  });

  // Custom date range attendance summary
  fastify.get('/custom-summary', { preHandler: [auth, rbac('ADMIN')] }, async (req) => {
    const { from, to } = req.query;
    if (!from || !to) throw new Error('from and to dates required');
